...
class SpatialAttention(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        concat = tf.concat([avg_pool, max_pool], axis=-1)  # shape: (batch, h, w, 2)
        attention = self.conv(concat)  # (batch, h, w, 1)
        return inputs * attention  # spatially weighted
...
callbacks = [
    ...
    ModelCheckpoint('best_model_spatial_attention.keras', monitor='val_mae', save_best_only=True),
    TensorBoard(log_dir='logs_spatial_attention')
]
...
print("Training finished! Model saved as best_model_spatial_attention.keras")
